...
def make_canvas_scene(fixture_id: Int = CANVAS_SCENE_PRIMITIVES) -> Scene:
    """Build one canonical non-plot scene shared by parity tests and export demos.

    This core helper only covers the portable-primitives and theme fixtures;
    it does not know about the plot model. Callers that need the
    `CANVAS_SCENE_PLOT` fixture use `moxi_plot.fixtures.make_canvas_scene`,
    which wraps this function and adds the plot-backed branch.
    """
    var scene = Scene()
