...
| Rendering | Paint commands feed an inspectable scene IR. Software rendering is deterministic and now has seven exact PPM goldens; AppKit is the main native UI renderer; Metal supports a substantial geometry/text/image/path slice and dense plot packets. Native scene replay and an offscreen screenshot tolerance check compare the same compact scene; a genuine visible AppKit capture has also been manually reviewed; pixels remain platform-dependent. | High for contracts; medium for parity | `paint.mojo`, `scene.mojo`, `software.mojo`, `macos.mojo`, `metal.mojo`, `tests/goldens/`, `tests/native_scene_parity.mojo`, `tests/native_screenshot_policy.json`, native sources and `/tmp/moxi-visible-appkit-current.mov` | retain visible-state review artifacts and explicit font/scale masks; do not imply pixel-identical native output |
...
| Tests and visual QA | Contract coverage is broad, the software renderer exposes deterministic checksums, and seven lossless PPM goldens run through `scripts/visual_check.sh` with reviewable mismatch artifacts. Native text and scene structural parity plus the offscreen screenshot tolerance policy run on macOS; a visible AppKit review lane has now been exercised, but it is not yet a pixel-parity gate. | High | `scripts/test.sh`, `scripts/visual_check.sh`, `scripts/native_screenshot_check.sh`, `tests/goldens/`, `tests/native_text_parity.mojo`, `tests/native_scene_parity.mojo`, `tests/native-screenshot-policy.json`, renderer tests, `docs/*.svg`, `/tmp/moxi-visible-appkit-current.mov` | add real device/browser automation and a linked Mojo Web runtime before claiming linked non-macOS support |
...
2. **Root-wide work.** The typed executor and opted-in `App` lane bound one
   ...
   scroll, sibling popup state, accessibility identity, pointer capture, and
   deeper nested state. Scope/dirty/keyed-child/dependency-edge lookups are
   indexed, but ordinary components still rebuild the root and the fallback
   remains the adoption boundary.
3. **Native visual parity.** The software oracle has exact goldens, the
   offscreen Metal path has a tolerance/mask report, and a visible AppKit
   capture has been reviewed. Platform fonts, repeated visible-state review,
   and device/browser automation remain host review lanes.
