# Moxi feature inventory

This is the full per-feature inventory behind the [README](../README.md) pitch:
every shipped capability, the exact 0.5/Core scope boundary, and the demo/example
catalog. Fallback and limitation notes are preserved alongside the capabilities
they qualify — they are load-bearing, not caveats to skim past.

## Data flow

Views are Mojo value types; a component owns application state, builds a
lightweight declarative view, and handles events. The current data flow is
intentionally small:
...
`Scene`/`SceneRenderer` path. [ARCHITECTURE.md](../ARCHITECTURE.md) documents the
implemented contracts and lifecycle in full.

## Runtime, components, and reconciliation
...
frame delta. Renderers default to complete-frame dispatch. A retained-surface
backend can override `supports_incremental()`; `App.render()` then clears
removed regions and submits only changed commands.
...
system copy/cut/paste synchronization is needed. For system clipboard
synchronization, pass a `MacOSClipboard` to
...
to descendant paint commands. Arrow keys navigate between semantic buttons and
text inputs, while left/right remain text editing keys when a single-line
input owns focus.
...
[../examples/hello_component.mojo](../examples/hello_component.mojo). For
explicit styling, use `add_label_styled()`, `add_button_styled()`,
`set_surface_style()`, and `set_panel()` on `ColumnView`.
...
and semantic-action paths share one update contract.
...
in [text-policy.md](text-policy.md); native scene checks compare structure and
fallback counters exactly while leaving raster pixels to a future
...
selection. `CompositionEvent("...", start, end)` carries transient marked text
without mutating the committed value. `CompositionEvent()` ends the current
...
The keyboard and text-input slice is in
[../examples/form.mojo](../examples/form.mojo) and uses the shared `FormState`
scenario, including selection, marked-text IME composition, and portable
clipboard commands.
...
`Scene`, `SceneCommand`, and `SceneRenderer` form a richer rendering seam;
`SoftwareSceneRenderer` is a deterministic raster backend and
`MacOSMetalRenderer` is the batched GPU path. Its ASCII, CoreText Unicode
texture, image, curve/arc, and concave-simple-path support is
...
written under `dist/visual-artifacts/` for CI review.
...
they change between rendered frames. The full accessibility/native-widget
contract is documented in [accessibility.md](accessibility.md).
...
offsets against content and viewport extents.
...
benchmarks. `pixi run scenario-check` verifies that the paths, Pixi tasks,
golden labels, and fixture metadata still agree.

An interaction lab renders and exercises stable-key collection/table, tree
disclosure, scrollbar geometry, nested popup layers, and pointer reorder state
in one live component (`pixi run interaction-showcase-demo`, also available as
`Collection & Interaction Lab` in the Moxi Playground).
...
its evidence is in [support-matrix.md](support-matrix.md).
...
The named iOS, Android, and Web bridges normalize host events and provide
fallback output. SDK-backed host artifacts are checked by `pixi run
host-check`; the Mojo package remains capability-gated for native availability
because it is currently published as an `osx-arm64` package.
...
`pixi run browser-check` exercises the served `native/web/host_demo.html`
lifecycle and host-side ARIA mapping without requiring a browser package
runtime; the page itself exposes readiness and Canvas state markers for a
...
`CapabilityBus` is an in-process authorization boundary: it does not pretend
to be a network transport or an LLM protocol. UI and agent adapters construct
the same `CapabilityInvocation`; destructive or policy-marked calls must carry
a bus-issued approval bound to the exact request, and asynchronous adapters
can hold an exclusive lease with `authorize()`/`complete()`. The bounded FIFO,
`manifest_json()`, and `ConversationContext.turn_payload()` cover the queue,
tool-manifest, and fresh turn-boundary seams without forcing a network client
or LLM session into the UI core. `invoke()` is deliberately executor-less and
returns `CAPABILITY_EXECUTOR_REQUIRED`; typed handlers use `invoke_handler()`.

`CapabilityBus` can sit beside component state as the policy boundary for
mutations. Register a `CapabilityDescriptor`, construct a
`CapabilityInvocation`, and call `authorize()` when the application owns the
mutation. Use `invoke_handler()` for a registered typed executor; plain
`invoke()` rejects executor-less work. Exclusive authorizations return a lease
token that must be supplied to `complete()`.
...
boundary. The full design note, including the seams reserved for a future
agent/transport bridge, is in
[capability-bus-design.md](capability-bus-design.md).
...
agent-integration contracts. The component surface includes labels, buttons,
single-line and multiline text inputs, checkbox/switch/radio controls,
determinate progress and slider controls, image/resource descriptors, combo
boxes, lists, tables, trees, menus, dialogs, tabs, canvases, separators,
spacers, nested containers, and typed component slots. The catalog state
models are deliberately small, but each kind has a portable semantic role and
a deterministic headless behavior path. The wx-style showcase is the shared
scenario for these controls, stable action routing, clipping, scroll state,
wrapped text, rich-text fallback reporting, backend capabilities, capability
authorization, and a typed embedded counter component.
...
- The current layout supports vertical columns, horizontal rows, stack/grid/
  ...
  the shared App event path. The animation API is frame-stepped; it does not
  provide a hidden platform scheduler.
...
- The core exposes dirty regions and changed commands, and `TestRenderer`
  ...
  even-odd compound/self-intersecting paths, while malformed/overlarge paths
  and unregistered resources remain explicit fallback behavior.
- The 0.5 public boundary is the APIs documented in the [README](../README.md)
  and in [ARCHITECTURE.md](../ARCHITECTURE.md); larger facilities remain
...
`pixi run check` generates compiler API metadata at `dist/moxi-api.json`,
checks the canonical scenario consumer inventory, runs the software visual
corpus, and runs the host lifecycle checks. `pixi run benchmark-quick` is the
portable smoke profile; `pixi run benchmark-full` adds the complete plot,
collection, fractal, and Metal matrix. Both profiles write structured JSON
under `dist/benchmark-results/`; see [benchmarking.md](benchmarking.md).
Reviewed release references and the host/dispersion policy plus clean-tree
refresh command live under [`../benchmarks/results/`](../benchmarks/results/).
...
release-preflight` with an explicit remote channel and Pixi auth file before
uploading public artifacts.
...
methodology and comparison caveats are in [performance.md](performance.md).

## Demos

The full `pixi run` task list (build, test, and demo/benchmark tasks):
...
`pixi run demo` and `pixi run demo-browser` both open the Moxi Playground,
inspired by wxPython's demo browser. Search or filter the catalog, read the
selected example's overview/source, and mount its real component page in the
same window. Ordinary component pages show a Storybook-style live preview next
to the component-usage code that mounts it; scene and plotting pages render
...
demonstrates linked selection. Press `Escape` to clear the search query.

The `Editable Live Component` watches `examples/editable_showcase.mojo`: save
that file in your editor and its exported scene is rebuilt and swapped into
the existing canvas. Close the window to end the event loop. `pixi run
live-reload-check` validates that build/load/render ABI without a GUI. See
[demo-browser.md](demo-browser.md#editable-source-and-hot-reload) for the full
editable-source workflow.

`pixi run hello-window-demo` retains the smallest native window example.
`pixi run capability-bus-demo` opens the ten-step walkthrough component. Its
Next, Previous, Restart, and agent approval controls all construct capability
invocations and execute them through a typed `CapabilityBus` handler. The
matching plain-text transcript and recording directions are in
[capability-bus-walkthrough.txt](capability-bus-walkthrough.txt). `pixi run
demo-record` produces the reproducible 30-second single-window capture from
the same normal host; it needs no agent or manual UI interaction. The
generated `dist/` files and native object file are local build artifacts.

The inline GIF below is an accelerated 10-second teaser of the normal live
demo window, with the capability bus driving the counter, plot, Metal, and
collection examples:

[![10-second Moxi Playground walkthrough](moxi-capability-bus-walkthrough-10s.gif)](moxi-capability-bus-walkthrough-10s.mp4)

For the full-speed version, watch or download the
[30-second walkthrough](moxi-capability-bus-walkthrough-30s.mov). Recreate the
source capture locally with `pixi run demo-record`.
...
height recomputation as the root width changes. `pixi run animation-demo` is a
deterministic, headless frame-clock lesson showing how `Animation` and
`Invalidation` compose without a platform timer.

`pixi run composed-demo` shows a parent component owning a typed counter
child; the child is embedded with namespaced ids and still handles its local
action.
...
`Collection & Interaction Lab` in the Playground.
...
selections. `pixi run plot-analytics-benchmark` repeats those recipes and
links stable-key selections. `pixi run plot-svg` prints the same scene as
Web-compatible SVG; `pixi run plot-large-benchmark` measures a 10k-point line
and bounded 100k scatter scene. `pixi run plot-stress-benchmark` runs the 1M-row
scatter stress case with a 50k geometry limit. `pixi run plot-metal-benchmark`
measures the packetized dense-mark path and complete plot composition.
`pixi run metal-window-demo-build` compiles the visible Metal window; run the
resulting binary locally when a GUI session is available.

The visual acceptance surfaces are listed in [visual.md](visual.md).
...
[../examples/counter.mojo](../examples/counter.mojo). It lays out a title,
count label, and button, then regenerates the view and command stream after
each click on `Increment` or root-size change. The smaller lifecycle example
is in [../examples/hello_component.mojo](../examples/hello_component.mojo).

For a source file that can be edited and reloaded while the host stays open,
see [../examples/editable_showcase.mojo](../examples/editable_showcase.mojo)
and [demo-browser.md](demo-browser.md#editable-source-and-hot-reload).

The reusable-control and horizontal-layout slice is in
[../examples/row.mojo](../examples/row.mojo) and uses the shared `RowState`
scenario. The first 0.5 layout slices are in
[../examples/alignment.mojo](../examples/alignment.mojo) and
[../examples/nested.mojo](../examples/nested.mojo); both use shared component
scenarios and are covered by headless tests.

The learning-oriented wxPython-style slice is in
[../examples/wx_style.mojo](../examples/wx_style.mojo) and uses `WxStyleState`
from [../src/moxi/wxstyle.mojo](../src/moxi/wxstyle.mojo). It keeps the
familiar `Frame -> Panel -> BoxSizer -> control` vocabulary while showing the
actual Moxi lifecycle: build, route, stable action, capability authorization,
update, rebuild, and render. It includes every currently shipped control kind,
stateful catalog interactions, a registered native image, scrolling, and the
typed embedded counter component.

## Roadmap

The plot foundation has typed data, executable statistical transforms,
...
cross-platform resource backends remain staged follow-up work. The
capability-bus design note is in
[capability-bus-design.md](capability-bus-design.md); the in-process
authorization boundary it describes is part of the Moxi core, and the larger
transport/agent bridge it anticipates is not yet implemented.
