...
| Scrolling | Bounded portal scrolling, persistent offsets, clipping-aware hit testing, fixed/variable-extent range math, a stable-key `VirtualRecycler`/`VirtualizedList` with measured heights, prefix offsets, overscan, anchor correction, ensure-visible behavior, and renderer-independent scrollbar geometry. Scrollbar painting and final viewport integration remain staged. | Mature virtual scrolling widgets and viewport integration. |
